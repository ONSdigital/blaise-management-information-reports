...
export async function setupAppointment(
  ...
  await casePage.waitForSelector("text=Selected timeslot: No timeslot selected", {
    state: "visible",
  });
  await casePage.click("#e-tbr-btn_3 > span.e-tbar-btn-text");
  await casePage
    .locator("table.e-schedule-table")
    .locator("tbody")
    .locator(`//tr/td[@data-date=${createDateForTomorrowAt1000()}]`)
...
export async function clearCatiData(
  page: Page,
  questionnaireName: string,
  userCredentials: NewUser,
): Promise<void> {
