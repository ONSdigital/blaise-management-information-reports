...
export async function setupTestUser(
  ...
  const userName = `dst-test-user-${randomUUID().slice(0, 8)}`;
  const user = {
    password: password,
    name: userName,
    role: "DST",
    serverParks: [serverPark],
...
export async function setupQuestionnaire(
  ...
  console.log(
    `Attempting to install and configure questionnaire ${questionnaireName} on server park ${serverPark}`,
  );
  await connectToRestApi(blaiseApiClient);
  ...
  console.log(
    `Installed and configured questionnaire ${questionnaireName} on server park ${serverPark}`,
  );
}
...
async function installQuestionnaire(
  blaiseApiClient: BlaiseApiClient,
  serverPark: string,
  questionnaireName: string,
) {
  try {
    ...
    for (let attempts = 0; attempts <= 12; attempts++) {
      const questionnaireDetails = await blaiseApiClient.getQuestionnaire(
        serverPark,
        `${questionnaireName}`,
      );
...
async function addSurveyDays(
  ...
  try {
    await blaiseApiClient.addSurveyDays(serverPark, `${questionnaireName}`, [
      today.toISOString(),
