...
export default function AppointmentResults({ messageNoData, reportData }: Props): ReactElement {
  return (
    <ErrorBoundary errorMessageText="Failed to load">
      {reportData && reportData.length > 0 ? (
        <table
          id="report-table"
          className="ons-table ons-elementToFadeIn ons-u-mt-s"
        >
          ...
          <tbody className="ons-table__body">
            {reportData.map((data: AppointmentResourcePlanningReportData) => (
              ...
              >
                <td className="ons-table__cell ">{data.questionnaire_name}</td>
                <td className="ons-table__cell ">{data.appointment_time}</td>
                <td className="ons-table__cell ">{data.appointment_language}</td>
                <td className="ons-table__cell ">{data.case_reference}</td>
                <td className="ons-table__cell ">{data.telephone_number?.replace(/'/g, "")}</td>
                <td className="ons-table__cell ">{data.respondent_name?.replace(/'/g, "")}</td>
              </tr>
            ))}
          </tbody>
        </table>
