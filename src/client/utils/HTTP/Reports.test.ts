    ...
    mockAdapter
      .onPost("/api/questionnaires", {
        asymmetricMatch: (body: unknown) => {
          ...
          expect(requestBody.survey_tla).toBe("DST");
          expect(requestBody.interviewer).toBe("James");
          expect(requestBody.start_date).toBe("2022-01-02");
          expect(requestBody.end_date).toBe("2022-02-05");
...
describe("getInterviewerCallHistoryReport", () => {
  ...
  it("posts the search parameters", async () => {
    expect.assertions(5);
    mockAdapter
      .onPost("/api/reports/interviewer-call-history", {
        asymmetricMatch: (body: unknown) => {
          const requestBody = body as Record<string, string>;
          ...
          expect(requestBody.start_date).toBe("2022-01-02");
          expect(requestBody.end_date).toBe("2022-02-05");
          expect(requestBody.questionnaires).toBe("INST1,INST2");
    ...
    mockAdapter
      .onPost("/api/reports/interviewer-call-history", {
        asymmetricMatch: (body: unknown) => {
          ...
          expect(requestBody.survey_tla).toBe("undefined");
          expect(requestBody.interviewer).toBe("undefined");
          expect(requestBody.start_date).toBe("undefined");
          expect(requestBody.end_date).toBe("undefined");
          expect(requestBody.questionnaires).toBe("undefined");
  ...
  it("returns the questionnaires", async () => {
    mockAdapter
      .onPost("/api/reports/interviewer-call-history")
      .reply(200, [testQuestionnaireResponse]);
    expect(await getInterviewerCallHistoryReport(testFormParameters)).toEqual([
