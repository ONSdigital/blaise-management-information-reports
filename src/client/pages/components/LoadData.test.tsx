...
describe("LoadData", () => {
  ...
  describe("initial state", () => {
    it("displays the loading spinner", () => {
      render(
        <LoadData dataPromise={new Promise(() => {}) as Promise<ReactNode>}>{display}</LoadData>,
      );
  ...
  describe("loaded state", () => {
    it("displays the loaded data", async () => {
      render(<LoadData dataPromise={Promise.resolve("Hello world!")}>{display}</LoadData>);
  ...
  describe("errored state", () => {
    it("does not display the loading spinner", async () => {
      render(
        <LoadData dataPromise={Promise.reject(new Error("There was an error"))}>
          {display}
        </LoadData>,
      );
  ...
  describe("dataPromise props changing", () => {
    ...
    beforeEach(async () => {
      const view = render(
        <LoadData dataPromise={Promise.resolve("Old content")}>{display}</LoadData>,
      );
    ...
    it("displays the new data when the dataPromise prop changes", async () => {
      rerender(<LoadData dataPromise={Promise.resolve("New content")}>{display}</LoadData>);
