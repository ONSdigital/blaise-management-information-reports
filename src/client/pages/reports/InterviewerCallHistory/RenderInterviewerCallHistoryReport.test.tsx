...
describe("RenderInterviewerCallHistoryReport", () => {
  ...
  it("posts the search parameters to the backend", async () => {
    ...
    expect(new Date(formData!.start_date as string)).toEqual(
      new Date("September 18, 2022 01:23:00"),
    );
  ...
  describe("content which is always present", () => {
    ...
    it("displays the filter summary", () => {
      ...
      expect(
        screen.getByRole("heading", { name: /Period:\s*18\/09\/2022\s*–\s*17\/10\/2022/ }),
      ).toBeVisible();
      expect(
        screen.getByRole("heading", { name: /Questionnaires:\s*LMS1111 and LMS2222/ }),
      ).toBeVisible();
    });
  });
  ...
  describe("when results are loaded", () => {
    ...
    it("displays the CVS link", () => {
      expect(
        screen.getByText(/Export report as Comma-Separated Values \(CSV\) file/),
      ).toBeVisible();
    });
