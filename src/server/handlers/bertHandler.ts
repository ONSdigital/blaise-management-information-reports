...
export default function newBertHandler(
  ...
  router.post(
    ...
    auth.middleware,
    bertHandler.getInterviewerCallHistoryReport,
  );
  router.post(
    "/api/reports/interviewer-call-pattern",
    auth.middleware,
    bertHandler.getInterviewerCallPattern,
  );
  router.post(
    "/api/reports/appointment-resource-planning",
    auth.middleware,
    bertHandler.getAppointmentResourcePlanningReport,
  );
  router.post(
    "/api/appointments/questionnaires",
    auth.middleware,
    bertHandler.getAppointmentQuestionnaires,
  );
  router.post(
    "/api/reports/appointment-resource-planning-summary",
    auth.middleware,
    bertHandler.getAppointmentResourcePlanningSummaryReport,
  );
...
class BertHandler {
  ...
  getCallHistoryStatus = async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      req.log.error(error, "Error fetching call history status from BERT");
  ...
  getAppointmentResourcePlanningReport = async (req: Request, res: Response): Promise<Response> => {
    ...
    const questionnairesQuery =
      questionnaires.length > 0 ? `&questionnaires=${questionnaires}` : "";
  ...
  getAppointmentResourcePlanningSummaryReport = async (
    req: Request,
    res: Response,
  ): Promise<Response> => {
    const { date, survey_tla: surveyTla, questionnaires } = req.body;
